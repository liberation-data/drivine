...
describe('AgensGraphSpecCompiler', () => {
    it('should map parameters to AgensGraph format', () => {
        ...
        const compiledSpec = new AgensGraphSpecCompiler(spec.finalizedCopy('CYPHER')).compile();
    ...
    it('should map parameters to AgensGraph format', () => {
        const cql = `MERGE (u:Urbanite { id : $id } ) set u += $urbaniteProps WITH u UNWIND $metros AS metro MERGE (m:Metro {name: metro}) WITH u, m, metro MERGE (u)-[:FREQUENTLY_HAUNTS]-(m)`;
        ...
        const spec = new QuerySpecification(cql).bind(params).finalizedCopy('CYPHER');
        const compiledSpec = new AgensGraphSpecCompiler(spec).compile();
        const map: any[] = compiledSpec.parameters;
        const statement = compiledSpec.statement;
        expect(statement).toEqual(
            'MERGE (u:Urbanite { id : $1 } ) set u += $3 WITH u UNWIND $2 AS metro MERGE (m:Metro {name: metro}) WITH u, m, metro MERGE (u)-[:FREQUENTLY_HAUNTS]-(m)'
        );
